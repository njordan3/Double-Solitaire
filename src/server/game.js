...
module.exports = class Game {
    ...
    placeCard(id, x, y) {
        ...
        if (this.checkStackRowCollision(id, x, y)) {
            for (let i = 0; i < 7; i++) {
                if (top_cards.stacks[i] != undefined && i != moving.stack && this.checkCardCollision(top_cards.stacks[i], x, y)) {
                    for (let j = 0; j < moving.cards.length; j++) {
                        this.decks[id].stacks[i].cards.up.push(this.decks[id][moving.type][moving.stack].cards.up.pop());
                    }
                    this.decks[id].stacks[i].alignCards();
                    break;
                }
            }
        } else if (this.checkAceRowCollision(x, y)) {
            for (let i = 0; i < 8; i++) {
                if (top_cards.aces[i] != undefined && this.checkCardCollision(top_cards.aces[i], x, y)) {
                    break;
                }
            }
        } else {
            // return moving cards to their original positions
            this.decks[id][moving.type][moving.stack].alignCards();
        }
        this.moving_cards[id] = {cards: []};
    }
    ...
    getTopCards(id) {
        ...
        for (let i = 0; i < 8; i++) {
            top_cards.aces[i] = this.aces[i].cards.up[this.aces[i].top('up')];
        }
        return top_cards;
    }
    toJSON() {
